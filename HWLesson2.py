...
check_quit = True
while check_quit:
    print("Введите два числа и операцию с ними (+, -, *, /) . Введите 0 как операцию для завершения.")
    while True:
        action = str(input("Введите операцию - "))
        if action not in operations.keys():
            ...
            if action == "/" and (b - round(b, 5)) < 0.00001:
                wrong_b = True
                while wrong_b:
                    print("При операции деления знаменатель не может быть равен 0")
                    b = float(input("Введите Второе число - "))
                    if (b - round(b, 5)) > 0.00001:
                        wrong_b = False
            print(operations[action](a, b))
            break
